...
TEST(ParallelAlgorithms, ParallelFill) {
    ...
    // 验证是否所有元素都被设置为5
    EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 5; }));
}
...
TEST(ParallelAlgorithms, ParallelRangeForSingle) {
    ...
    // 验证是否所有元素都被设置为2
    EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 2; }));
}
TEST(ParallelAlgorithms, ParallelForDouble) {
    ...
    for (const auto& row : matrix) {
        EXPECT_TRUE(std::all_of(row.begin(), row.end(), [](int i) { return i == 3; }));
    }
}
TEST(ParallelAlgorithms, ParallelRangeForDouble) {
    ...
    bool allFour = true;
    for (const auto& row : matrix) {
        if (!std::all_of(row.begin(), row.end(), [](int i) { return i == 4; })) {
            allFour = false;
            break;
        }
    }
...
TEST(ParallelAlgorithms, ParallelRangeForWithPolicy) {
    ...
    // 验证是否所有元素都被设置为6
    EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 6; }));
}
...
TEST(ParallelAlgorithms, ParallelForDoubleWithPolicy) {
	...
	// 验证是否所有元素都被设置为7
    for (const auto& row : matrix) {
		EXPECT_TRUE(std::all_of(row.begin(), row.end(), [](int i) { return i == 7; }));
	}
}
...
TEST(ParallelAlgorithms, ParallelRangeForDoubleWithPolicy) {
	...
	bool allEight = true;
    for (const auto& row : matrix) {
        if (!std::all_of(row.begin(), row.end(), [](int i) { return i == 8; })) {
			allEight = false;
			break;
		}
	}
...
TEST(ParallelAlgorithms, ParallelForWithPolicyParallel) {
	...
	// 验证是否所有元素都被设置为9
	EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 9; }));
}
...
TEST(ParallelAlgorithms, ParallelRangeForWithPolicyParallel) {
	...
	// 验证是否所有元素都被设置为10
	EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 10; }));
}
...
TEST(ParallelAlgorithms, ParallelForDoubleWithPolicyParallel) {
	...
	// 验证是否所有元素都被设置为11
    for (const auto& row : matrix) {
		EXPECT_TRUE(std::all_of(row.begin(), row.end(), [](int i) { return i == 11; }));
	}
}
...
TEST(ParallelAlgorithms, ParallelRangeForDoubleWithPolicyParallel) {
	...
	bool allTwelve = true;
    for (const auto& row : matrix) {
        if (!std::all_of(row.begin(), row.end(), [](int i) { return i == 12; })) {
			allTwelve = false;
			break;
		}
	}
...
TEST(ParallelAlgorithms, ParallelForWithPolicySingle) {
	...
	// 验证是否所有元素都被设置为13
	EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 13; }));
}
...
TEST(ParallelAlgorithms, ParallelRangeForWithPolicySingle) {
	...
	// 验证是否所有元素都被设置为14
	EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 14; }));
}
...
TEST(ParallelAlgorithms, ParallelForDoubleWithPolicySingle) {
	...
	// 验证是否所有元素都被设置为15
    for (const auto& row : matrix) {
		EXPECT_TRUE(std::all_of(row.begin(), row.end(), [](int i) { return i == 15; }));
	}
}
...
TEST(ParallelAlgorithms, ParallelRangeForDoubleWithPolicySingle) {
	...
	for (const auto& row : matrix) {
		if (!std::all_of(row.begin(), row.end(), [](int i) { return i == 16; })) {
			allSixteen = false;
			break;
		}
	}
...
TEST(ParallelAlgorithms, ParallelForWithPolicyParallelSingle) {
	...
	// 验证是否所有元素都被设置为17
	EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 17; }));
}
...
TEST(ParallelAlgorithms, ParallelRangeForWithPolicyParallelSingle) {
	...
	// 验证是否所有元素都被设置为18
	EXPECT_TRUE(std::all_of(vec.begin(), vec.end(), [](int i) { return i == 18; }));
}

TEST(ParallelAlgorithms, ParallelSort) {
